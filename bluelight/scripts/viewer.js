...
class LoadFileInBatches {
  ...
  static handleError(error, url) {
    ...
    showErrorMessage(`There's an error with PACS server: ${error.message}`);
    LoadFileInBatches.finishOne();
  }

  static wadoPreLoad(url, onlyload) {
    if (LoadFileInBatches.NumOfFetchs === 0) {
      showLoading(); // Show loading UI when starting first fetch  
    }
  ...
  static finishOne() {
    if (LoadFileInBatches.NumOfFetchs > 0) {
      ...
      if (LoadFileInBatches.NumOfFetchs === 0 && LoadFileInBatches.queue.length === 0) {
        hideLoading(); // Hide loading UI when all fetches are done  
      }
    }
  }
}
...
function wadorsLoader2(url, onlyload) {  
  ...
  fetch(url, { headers, })  
    ...
    .finally(() => {  
      LoadFileInBatches.finishOne();  
      // Hide loading UI if implemented and all fetches are done  
      if (LoadFileInBatches.NumOfFetchs === 0 && typeof hideLoading === 'function') {  
        hideLoading();  
      }  
    });  
}
...
function loadDICOMFromUrl(url, loadimage = true) {
  ...
  fetch(url, { headers, })
    .then(async function (res) {
      if (!res.ok) {
        throw new Error(`Failed to access PACS server: ${res.status} ${res.statusText}`);
      }
