...
function drawAnatomicalPlane(obj) {
    ...
    try {
        ...
        var corners = [
            [0, 0, 0],        // 左上角
            [GetViewport(z).content.image.width, 0, 0],      // 右上角
            [0, GetViewport(z).content.image.height, 0],      // 左下角
            [GetViewport(z).content.image.width, GetViewport(z).content.image.height, 0]     // 右下角
        ];
        var transCor = corners.map(corner => Matrix4x4.applyMatrixToCoordinate(GetViewport(z).content.image.RCS.matrix, corner));
        var transCor = transCor.map(corner => Matrix4x4.applyMatrixToCoordinate(GetViewport().content.image.RCS.invertmatrix, corner));
        var x = 0, y = 1, offsetX = 0, offsetY = 0, invertX = 1, invertY = 1;
        ...
        if (element.content.image.AnatomicalPlane === 'Sagittal' && GetViewport(z).content.image.AnatomicalPlane == 'Axial') {
            [x, y] = [0, 2];
            [offsetX, offsetY] = [GetViewport(z).content.image.width / 2, GetViewport(z).content.image.height];//[GetViewport(z).content.image.width / 2, 0];
            invertX = -1;
        }
        else if (element.content.image.AnatomicalPlane === 'Sagittal' && GetViewport(z).content.image.AnatomicalPlane == 'Coronal') {
            [x, y] = [0, 1];
            [offsetX, offsetY] = [GetViewport(z).content.image.width / 2, 0]//[GetViewport(z).content.image.width / 2, GetViewport(z).content.image.height];
            invertX = -1;
        }
        else if (element.content.image.AnatomicalPlane === 'Axial' && GetViewport(z).content.image.AnatomicalPlane == 'Sagittal') {
            [x, y] = [1, 2];
            [offsetX, offsetY] = [0, GetViewport(z).content.image.height / 2];//[GetViewport(z).content.image.width, GetViewport(z).content.image.height / 2];
            //invertY = -1;
        }
            ...
            [x, y] = [2, 1];
            [offsetX, offsetY] = [GetViewport(z).content.image.width, GetViewport(z).content.image.height / 2];//[0, GetViewport(z).content.image.height / 2];
            invertY = -1;
        }
